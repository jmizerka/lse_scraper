...
def parse_args():
    ...
    parser.add_argument(
        "--input",
        required=True,
        help="Path to the input CSV file containing stock symbols and names."
    )
    parser.add_argument(
        "--output",
        required=True,
        help="Path where the output CSV with stock results will be saved."
    )
    parser.add_argument(
        "--cron",
        default="*/5 * * * *",
        help="Cron expression defining the schedule (default: every 5 minutes)."
    )
